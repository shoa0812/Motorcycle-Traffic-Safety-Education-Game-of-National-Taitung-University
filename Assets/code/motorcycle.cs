...
public class Motorcycle : MonoBehaviour { //MS Motorcycle test - Marcos Schultz (www.schultzgames.com)
    ...
    void GenerateWheels(){
        ...
        //
        GameObject frontWheelModel = GameObject.Find("frontWheel"); // 假設你的前輪的名字是 "frontWheel"
        if (frontWheelModel != null) {
            ...
            Debug.LogError("找不到前輪物件，請確保場景中存在名為 frontWheel 的物件");
        }
        frontWheel = GameObject.Find("frontWheel").GetComponent<WheelCollider>();
        //生成後輪
        ...
        if (rearWheelModel != null) {
            ...
            Debug.LogError("找不到後輪物件，請確保場景中存在名為 rearWheel 的物件");
        }
        rearWheel = GameObject.Find("rearWheel").GetComponent<WheelCollider>();
